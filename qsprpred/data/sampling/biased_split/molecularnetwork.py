...
def visualise_molnet(G, filepath=None):
    ...
    ax.axis("off")
    plt.title(
        f"Molecular Network with {G.number_of_nodes()} molecules & "
        f"{G.number_of_edges()} edges made using \n"
        f"Similarity Threshold of {G.graph['similarity_threshold']} over "
        f"{G.graph['similarity_fp']} fingerprints using "
        f"{G.graph['similarity_distance']} Similarity"
    )
    if filepath:
        plt.tight_layout()
...
def visualise_molnet_split(
    ...
    caption = (
        ...
        f"{G.graph['similarity_fp']}, {G.graph['similarity_distance']} "
        f"\u2265 {G.graph['similarity_threshold']}. "
